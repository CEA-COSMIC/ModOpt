"""Base Class for optimisations algorithms."""
...
class SetUp(Observable):
    ...
    metrics: dict
        Metric API dictionnary
    step_size:
    metric_call_period: int
        Period (in iteration) on which to call the metric API.
    compute_backend: str
        Currently numpy cupy and tensorflow are supported.
    xp: module
        module associated to the compute backend (e.g. np, cp, tf)
    """
    ...
    def __init__(
        ...
        **dummy_kwargs,
        ...
        metrics : dict, optional
            Metrics to be used (default is ``dict()``)
        ...
        compute_backend: str, default 'numpy'
            Compute backend for the setup, Either 'numpy' or 'cupy'
        """
        self.converge = False
